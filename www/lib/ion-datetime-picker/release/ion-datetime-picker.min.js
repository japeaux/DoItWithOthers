...
.directive("ionDatetimePicker",function(){
	return{
		restrict:"AE",require:"ngModel",
		scope:{modelDate:"=ngModel",title:"=?",subTitle:"=?",buttonOk:"=?",buttonCancel:"=?",monthStep:"=?",hourStep:"=?",minuteStep:"=?",secondStep:"=?",onlyValid:"=?"},
		controller:["$scope","$ionicPopup","$ionicPickerI18n","$timeout",function(e,t,n,i){
			function a(e,t){
				t=t||0,0!==t&&e.setDate(e.getDate()+t),
				l.year=e.getFullYear(),
				l.month=e.getMonth(),
				l.day=e.getDate(),
				l.hour=e.getHours(),
				l.minute=e.getMinutes(),
				l.second=e.getSeconds(),
				l.date=e
			}
			function o(){
			...
			e.i18n=n,e.bind={},
			e.rows=[0,1,2,3,4,5],e.cols=[1,2,3,4,5,6,7],
			e.weekdays=[0,1,2,3,4,5,6];
			var l={
				year:e.year,month:e.month,day:e.day,hour:e.hour,minute:e.minute,second:e.second,
				date:new Date,getDateWithoutTime:function(){
					var e=new Date(this.date);return e.setHours(0,0,0,0,0),e
				}
			};
			e.showPopup=function(){
				t.show({
					templateUrl:"lib/ion-datetime-picker/src/picker-popup.html",
					title:e.title||"Escolha "+(e.dateEnabled?"a data":"")+(e.dateEnabled&&e.timeEnabled?" e ":"")+(e.timeEnabled?"a hora":""),
					subTitle:e.subTitle||"",
					scope:e,cssClass:"ion-datetime-picker-popup",
					buttons:[{text:e.buttonOk||e.i18n.ok,type:e.i18n.okClass,onTap:function(){e.commit()}},
					{text:e.buttonCancel||e.i18n.cancel,type:e.i18n.cancelClass,onTap:function(){i(function(){e.processModel()},200)}}
					]})},
				e.prepare=function(){e.mondayFirst&&e.weekdays.push(e.weekdays.shift())},
				e.processModel=function(){
					...
					e.second=e.secondsEnabled?t.getSeconds():0,s()
				};
				var s=function(){
					o();
					var t=new Date(e.year,e.month,e.day,e.hour,e.minute,e.second);
					e.dateEnabled&&(e.year=t.getFullYear(),
						...
						e.daysInMonth=d(e.year,e.month)),
						...
						e.changeBy=function(t,n){
							if(+t){
								if(("hour"===n||"minute"===n)&&t===-1){
									...
									0!==e.minute&&"hour"!==n||e.hour!==i.getHours()||e.hour--}e[n]+=+t,"month"!==n&&"year"!==n||(e.day=Math.min(e.day,d(e.year,e.month))),s()}},
									e.change=function(t){var n=e.bind[t];
										...
										e.changeDay=function(t){e.day=t,s()},e.isEnabled=function(t,n){if(!e.onlyValid)return!0;var i=new Date(e.year,e.month,t),o=e.onlyValid;
											o instanceof Array||(o=[o]);for(var s=!0,d=0;d<o.length;d++){var r=o[d];
												if(r.after){var u=c(r.after);r.inclusive?(s=i>=u,!s&&n&&a(u,0)):(s=i>u,!s&&n&&a(u,1))
												}else if(r.before){var m=c(r.before);r.inclusive?(s=i<=m,!s&&n&&a(m,0)):(s=i<m,!s&&n&&a(m,-1))
												}else if(r.between){var b=c(r.between.initial),h=c(r.between["final"]);
												r.inclusive?(s=i>=b&&i<=h,!s&&n&&(i<b&&a(b,0),i>h&&a(h,0))):(s=i>b&&i<h,!s&&n&&(i<=b&&a(b,1),i>=h&&a(h,-1)))
											}else if(r.outside){var b=c(r.outside.initial),h=c(r.outside["final"]);
											if(r.inclusive){if(s=i<=b||i>=h,!s&&n){var g=l.getDateWithoutTime();
												g<=b&&a(h,0),g>=h&&a(b,0)}}else if(s=i<b||i>h,!s&&n){var g=l.getDateWithoutTime();
													g<b&&a(h,1),g>h&&a(b,-1)}}if(!s)break}return s},
													e.changed=function(){s()},
													e.dateEnabled&&e.$watch(function(){return(new Date).getDate()},function(){var t=new Date;
														e.today={day:t.getDate(),month:t.getMonth(),year:t.getFullYear()}})}],link:function(e,t,n,i){e.dateEnabled="date"in n&&"false"!==n.date,e.timeEnabled="time"in n&&"false"!==n.time,e.dateEnabled===!1&&e.timeEnabled===!1&&(e.dateEnabled=e.timeEnabled=!0),
													e.mondayFirst="mondayFirst"in n&&"false"!==n.mondayFirst,e.secondsEnabled=e.timeEnabled&&"seconds"in n&&"false"!==n.seconds,e.meridiemEnabled=e.timeEnabled&&"amPm"in n&&"false"!==n.amPm,e.monthStep=+e.monthStep||1,e.hourStep=+e.hourStep||1,e.minuteStep=+e.minuteStep||1,e.secondStep=+e.secondStep||1,
													e.prepare(),i.$render=function(){e.modelDate=i.$viewValue,e.processModel()},e.commit=function(){e.modelDate=new Date(e.year,e.month,e.day,e.hour,e.minute,e.second),i.$setViewValue(e.modelDate)},
													t.on("click",e.showPopup)}}}),

angular.module("ion-datetime-picker").factory("$ionicPickerI18n",["$window",function(e){
	return{
		ok:"OK",cancel:"Cancel",okClass:"button-positive",cancelClass:"button-stable",arrowButtonClass:"button-positive",weekdays:e.moment?e.moment.weekdaysMin():["Do","Se","Te","Qu","Qu","Se","Sa"],
		months:e.moment?e.moment.months():["Janeiro","Fevereiro","Março","Abril","Maio","Junho","Julho","Agosto","Setembro","Outubro","Novembro","Dezembro"]}}]),
